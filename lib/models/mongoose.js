...
let iutil = require('../utils/inflector');
...
module.exports = function(connectors) {
  ...
  let loadModel = function(filePath) {
    ...
    let modelName = modelData.modelName || iutil.toModel(filePath);
    let tableName = modelData.tableName || iutil.toTable(modelName);
  ...
  let loadDiscriminator = function(filePath) {
    ...
    let modelData = require(projectModelsPath+"/"+iutil.toFile(discriminatorData.discriminator));
    ...
    let discriminatorModelName = discriminatorData.modelName || iutil.toModel(filePath);
    // registering mongoose model
